...
class Ernie4_5_VLMoeModel(nn.Module):
    hf_to_vllm_mapper = WeightsMapper(
        orig_to_new_stacked={
            ...
            ".k_proj": (".qkv_proj", "k"),
            ".v_proj": (".qkv_proj", "v"),
            # Routed experts are renamed by `_preprocess` and loaded by
            # `RoutedExperts.load_weights`, so they never match these keys.
    ...
    def _preprocess(
        ...
        """Split the checkpoint's single expert stack into text/vision experts.

        The checkpoint stores text and vision experts contiguously under
        `mlp.experts.{i}`, with the first `moe_num_experts[0]` being text
        experts; vLLM keeps them as two separate modules, so the vision half is
        re-indexed from zero. The routers are stored transposed under a shared
        `mlp.gate` name.
        """
        ...
        for name, loaded_weight in weights:
            if "mlp.experts" in name:
                ...
                if moe_offset < vision_expert_start_idx:
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        loader = AutoWeightsLoader(
            self,
            skip_substrs=["mtp"],
            # Extra biases carried by GPTQ checkpoints.
...
class Ernie4_5_VLMoeForCausalLM(nn.Module, SupportsPP):
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        skip_prefixes = ["vision_model.", "resampler_model."]
        if self.config.tie_word_embeddings:
